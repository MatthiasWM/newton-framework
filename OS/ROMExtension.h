...
const ULong kROMConfigTag				= (ULong) 'rom ';	// used by REx tool
...
const ULong kFlashConfigTag			= (ULong) 'flsh';	// used by REx tool
...
# endif
# ifdef OMHACK
extern const size_t ROM$$Size;
# endif
#define kRExPAddr0 ((PAddr)&ROM$$Size)

const PAddr kRExPAddr1 = (PAddr)(8 * MByte);
...
struct RExHeader
{
	...
	ULong start;				// virtual address of the top of this block
	ULong count;				// number of config entries
	ConfigEntry table[];		// table of config entries, will be more than one
};
...
const ULong kROMConfigTableVersion = 1;

struct ROMConfigEntry
{
	ULong romPhysStart;
	ULong romSize;
};
...
struct RAMAllocTable
{
	ULong version;				// version of this table format
	ULong count;				// count of entries in the table
	RAMAllocEntry table[];	// first three are required, but could be more
};
...
//----------------------------------------------------------------------
# if defined(hasCirrus) || defined(hasVoyager) || defined(forMac)
const ULong kFlashDriverVersion = 1;
class	CClassInfo;
struct SFlashDriverEntry
{
	ULong				fVersion;				// version of this table format
	CClassInfo *	fDriverClassInfo;		// pointer to the class info for flash driver
};
/*----------------------------------------------------------------------------*/
# else
...
struct FlashConfigEntry
{
	UChar manufacturer;
	UChar deviceCode;
	UChar internalDeviceCode;
	UChar needsVpp;
	ULong size;
	ULong blockSize;
	ULong start;
...
struct FlashBankConfigTable
{
	ULong version;				// version of this table format
	ULong count;				// count of entries in the table
	PAddr table[];				// starting physical address of flash chip
};
