...
#define PRINTF(args) printf("%s",gFnIndentStr+kFnIndentStrLen-gFnDepth); printf args
/* Newton Research additions */
#define kFnIndentStrLen 80
extern char gFnIndentStr[kFnIndentStrLen+1];
extern int gFnDepth;
#define ENTER_FUNC ++gFnDepth;
#define EXIT_FUNC if (gFnDepth > 0) --gFnDepth;
...
#define THROWIF(result, exception)			do { if (result) Throw((exception), (void*)(long)(result)); } while (0)
#define THROWNOT(expr, exception, result)	do { if (!(expr)) Throw((exception), (void*)(long)(result)); } while (0)
...
#define ASSERTMSG(expr, msg)	do { if (!(expr)) ThrowMsg(#msg); } while (0)
#define ASSERTTHROW(expr)		do { if (!(expr)) ThrowMsg("ASSERTTHROW(" #expr ") failed"); } while (0)
