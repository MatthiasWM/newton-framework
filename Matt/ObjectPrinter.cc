...
void ObjectPrinter::PrintFrame(RefArg ref) {
  ...
  if (optionDecompile_ && IsFunctionFrame(ref)) {
    ...
    if (theClass == kPlainCFunctionClass)
      Printf(" /** Can't decompile PlainCFunctionClass %s **/\n", this->RefPath().c_str());
    else if (theClass == kBinCFunctionClass)
      Printf(" /** Can't decompile binCFunction %s **/\n", this->RefPath().c_str());
    else if (EQ(theClass, SYMA(binCFunction)))
      Printf(" /** Can't decompile NOS 1.x binCFunction %s **/\n", this->RefPath().c_str());
    else
...
void ObjectPrinter::BuildRefMapForFunc(RefArg func)
{
  ...
  Ref theClass = GetArraySlot(func, 0);
	if (theClass == kPlainFuncClass) {
    fprintf(stderr, "Function is a plain function: %s\n", this->RefPath().c_str());
  } else if (theClass == kPlainCFunctionClass) {
    fprintf(stderr, "Function is a plain C function: %s\n", this->RefPath().c_str());
  } else if (theClass == kBinCFunctionClass) {
    fprintf(stderr, "Function is a binary C function: %s\n", this->RefPath().c_str());
  } else if (EQ(theClass, SYMA(CodeBlock))) {
    fprintf(stderr, "Function is a code block: %s\n", this->RefPath().c_str());
  } else if (EQ(theClass, SYMA(binCFunction))) {
    fprintf(stderr, "Function is a binary C function (SYMA): %s\n", this->RefPath().c_str());
  }
  if (IsArray(func)) {
    fprintf(stderr, "Function is an array: %s\n", this->RefPath().c_str());
    return;
  }
  */

  // Don't print the arg frame early, even if it used multiple times. It will
  // be generated by the compiler later.
